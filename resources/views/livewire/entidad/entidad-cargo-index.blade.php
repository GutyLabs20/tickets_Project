    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="flex space-x-2 justify-between mb-2">
            ...
            <div>
                ...
                @livewire('entidad.entidad-cargo-create')
                            ...
                            <tr class="bg-white">
                                ...
                                <td class="p-3 text-sm text-gray-700 whitespace-nowrap">
                                    <span
                                        ...
                                        {{ $cargo->activo == 1 ? 'Habilitado' : 'Deshabilitado' }}
                                    </span>
                                </td>
                                <td class="p-3 text-sm text-gray-700 whitespace-nowrap">
                ...
                @foreach ($cargos as $cargo)
                    <div class="bg-white space-y-3 p-4 rounded-lg shadow">
                        <div class="flex justify-between space-x-2 text-sm">
                            <div>
                                <a href="#" class="text-blue-500 font-bold hover:underline">{{ $cargo->id }}</a>
                            </div>
                            <div class="text-gray-500">{{ $cargo->nombre }}</div>
                            <div>
                                <span
                                    class="p-1.5 text-xs font-medium uppercase tracking-wider text-green-800 bg-green-200 rounded-lg bg-opacity-50">
                                    {{ $cargo->activo == 1 ? 'Habilitado' : 'Deshabilitado' }}
    ...
    <x-jet-dialog-modal wire:model="modal_edit">
        ...
        <x-slot name="content">
            <div class="col-span-6 sm:col-span-4">
                <x-jet-label for="nombre" value="{{ __('Position') }}" />
                <x-jet-input wire:model.defer="cargo.nombre" type="text" class="mt-1 block w-full"
                    />
                <x-jet-input-error for="cargo.nombre" class="mt-2" />
            </div>
            <div class="col-span-6 sm:col-span-4 mt-4">
                <x-jet-label for="descripcion" value="{{ __('Description') }}" />
                <x-jet-input wire:model.defer="cargo.descripcion" type="text" class="mt-1 block w-full"
                    />
                <x-jet-input-error for="cargo.descripcion" class="mt-2" />
            </div>
