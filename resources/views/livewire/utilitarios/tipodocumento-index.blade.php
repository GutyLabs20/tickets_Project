<div class="p-5 h-screen bg-gray-100">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="flex space-x-2 justify-between mb-2">
            ...
            <div>
                ...
                @livewire('utilitarios.tipodocumento-create')
                ...
                <table class="w-full">
                    <thead class="bg-gray-50 border-b-2 border-gray-200">
                        <tr>
                            ...
                            <th class="p-3 text-sm font-semibold tracking-wide text-left">{{ __('Document Type') }}</th>
                            ...
                            <tr class="bg-white">
                                ...
                                <td class="p-3 text-sm text-gray-700 whitespace-nowrap">
                                    <span
                                        ...
                                        {{ $documento->activo == 1 ? 'Habilitado' : 'Deshabilitado' }}
                                    </span>
                                </td>
                                <td class="p-3 text-sm text-gray-700 whitespace-nowrap">
                                    <div class="flex items-center justify-center">
                                        <div class="inline-flex shadow-md hover:shadow-lg focus:shadow-lg" role="group">
                                            ...
                                            <button type="button" wire:click="editar( {{ $documento }} )"
                                            ...
                                            <button type="button" wire:click="saveDelete( {{ $documento }} )"
                ...
                @foreach ($documentos as $documento)
                    <div class="bg-white space-y-3 p-4 rounded-lg shadow">
                        <div class="flex justify-between space-x-2 text-sm">
                            <div>
                                <a href="#" class="text-blue-500 font-bold hover:underline">{{ $documento->id }}</a>
                            </div>
                            <div class="text-gray-500">{{ $documento->nombre }}</div>
                            <div>
                                <span
                                    class="p-1.5 text-xs font-medium uppercase tracking-wider text-green-800 bg-green-200 rounded-lg bg-opacity-50">
                                    {{ $documento->activo == 1 ? 'Habilitado' : 'Deshabilitado' }}
    ...
    <x-jet-dialog-modal wire:model="modal_edit">
        <x-slot name="title">
            {{ __('Edit') }} {{ __('Document Type') }}
        </x-slot>

        <x-slot name="content">
            <div class="col-span-6 sm:col-span-4">
                <x-jet-label for="nombre" value="{{ __('Document Type') }}" />
                <x-jet-input wire:model.defer="documento.nombre" type="text" class="mt-1 block w-full"
                    />
                <x-jet-input-error for="documento.nombre" class="mt-2" />
            </div>
            <div class="col-span-6 sm:col-span-4 mt-4">
                <x-jet-label for="descripcion" value="{{ __('Description') }}" />
                <x-jet-input wire:model.defer="documento.descripcion" type="text" class="mt-1 block w-full"
                    />
                <x-jet-input-error for="documento.descripcion" class="mt-2" />
            </div>
