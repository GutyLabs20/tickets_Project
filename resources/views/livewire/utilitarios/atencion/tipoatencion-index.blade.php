<div class="p-5 h-screen bg-gray-100">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="flex space-x-2 justify-between mb-2">
            ...
            <div>
                ...
                @livewire('utilitarios.atencion.tipoatencion-create')
                ...
                <table class="w-full">
                    <thead class="bg-gray-50 border-b-2 border-gray-200">
                        <tr>
                            ...
                            <th class="p-3 text-sm font-semibold tracking-wide text-left">{{ __('Categorization') }}</th>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @foreach ($atenciones as $atencion)
                            <tr class="bg-white">
                                ...
                                <td class="p-3 text-sm text-gray-700 whitespace-nowrap">
                                    @if ($atencion->activo == 1)
                                        <span
                                            class="p-1.5 text-xs font-medium uppercase tracking-wider text-green-800 bg-green-200 rounded-lg bg-opacity-50">
                                            {{ 'Activo' }}
                                        </span>
                                    @endif
                                    @if ($atencion->activo == 0)
                                        <span
                                ...
                                <td class="p-3 text-sm text-gray-700 whitespace-nowrap">
                                    <div class="flex items-center justify-center">
                                        <div class="inline-flex shadow-md hover:shadow-lg focus:shadow-lg" role="group">
                                            ...
                                            <button type="button" wire:click="editar( {{ $atencion }} )"
                                            ...
                                            <button type="button" wire:click="saveDelete( {{ $atencion }} )"
                ...
                @foreach ($atenciones as $atencion)
                    <div class="bg-white space-y-3 p-4 rounded-lg shadow">
                        <div class="flex justify-between space-x-2 text-sm">
                            <div>
                                <a href="#" class="text-blue-500 font-bold hover:underline">{{ $atencion->id }}</a>
                            </div>
                            <div class="text-gray-500">{{ $atencion->nombre }}</div>
                            <div>
                                @if ($atencion->activo == 1)
                                    <span
                                        class="p-1.5 text-xs font-medium uppercase tracking-wider text-green-800 bg-green-200 rounded-lg bg-opacity-50">
    ...
    <x-jet-dialog-modal wire:model="modal_edit">
        <x-slot name="title">
            {{ __('Edit') }} {{ __('Categorization') }}
        </x-slot>

        <x-slot name="content">
            <div class="col-span-6 sm:col-span-4">
                <x-jet-label for="nombre" value="{{ __('Categorization') }}" />
                <x-jet-input wire:model.defer="atencion.nombre" type="text" class="mt-1 block w-full"
                    />
                <x-jet-input-error for="atencion.nombre" class="mt-2" />
            </div>
            <div class="col-span-6 sm:col-span-4 mt-4">
                <x-jet-label for="descripcion" value="{{ __('Description') }}" />
                <x-jet-input wire:model.defer="atencion.descripcion" type="text" class="mt-1 block w-full"
                    />
                <x-jet-input-error for="atencion.descripcion" class="mt-2" />
            </div>
